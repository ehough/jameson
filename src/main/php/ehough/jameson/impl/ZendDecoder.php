...
final class ehough_jameson_impl_ZendDecoder extends ehough_jameson_impl_AbstractDecoder implements ehough_jameson_api_IDecoder
{
    ...
    public function __construct($source = '')
    {
        ...
        // Set defaults
        $this->_source       = self::_decodeUnicodeString($source);
    ...
    private function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                ...
                throw new ehough_jameson_api_exception_RuntimeException('Missing "," in array encoding: ' . $this->_source);
            }
     ...
     * @throws ehough_jameson_api_exception_RuntimeException
     */
    private function _decodeObject()
    {
        $members = array();
        $tok     = $this->_getNextToken();
        ...
        while ($tok && $tok != self::RBRACE) {
            ...
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                ...
                throw new ehough_jameson_api_exception_RuntimeException('Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                ...
                throw new ehough_jameson_api_exception_RuntimeException('Missing ":" in object encoding: ' . $this->_source);
            }

            /** @noinspection PhpUnusedLocalVariableInspection */
            $tok           = $this->_getNextToken();
            $members[$key] = $this->_decode();
            $tok           = $this->_token;
    ...
    private function _getNextToken()
    {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            case '\'' :
                                ...
                                break;
                            default:
                                throw new ehough_jameson_api_exception_RuntimeException("Illegal escape sequence '{$chr}'");
                        }
                    } elseif($chr == '"') {
                        break;
                    } else {
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        throw new ehough_jameson_api_exception_RuntimeException("Octal notation not supported by JSON (value: {$datum})");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    throw new ehough_jameson_api_exception_RuntimeException("Illegal number format: {$datum}");
                }
            ...
            throw new ehough_jameson_api_exception_RuntimeException('Illegal Token');
        }
     ...
     */
    private function _eatWhitespace()
    {
        if (preg_match(
            '/([\t\b\f\n\r ])*/s',
            $this->_source,
     ...
     * @return string
     */
    private static function _decodeUnicodeString($chrs)
    {
        $chrs        = (string)$chrs;
        /** @noinspection PhpUnusedLocalVariableInspection */
